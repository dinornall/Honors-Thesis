...
"FATAL"       AbortLevel      - Error level when simulation should abort (string) {"WARNING", "SEVERE", "FATAL"}
       ...
       1e-3   ConvTol         - Convergence iteration error tolerance for tight coupling generalized alpha integrator (-)
          8   MaxConvIter     - Maximum number of convergence iterations for tight coupling generalized alpha integrator (-)
...
---------------------- INPUT FILES ---------------------------------------------
"MHK_RM1_Floating_ElastoDyn_R1.dat"   EDFile          - Name of file containing ElastoDyn input parameters (quoted string)
...
False         CalcSteady      - Calculate a steady-state periodic operating point before linearization? [unused if Linearize=False] (flag)
       ...
       0.01   TrimGain        - Proportional gain for the rotational speed error (>0) [used only if CalcSteady=True] (rad/(rad/s) for yaw or pitch; Nm/(rad/s) for torque)
        ...
        0.5   LinTimes        - List of times at which to linearize (s) [1 to NLinTimes] [used only when Linearize=True and CalcSteady=False]
...
False         VTK_fields      - Write mesh fields to VTK data files? (flag) {true/false} [unused if WrVTK=0]
         10   VTK_fps         - Frame rate for VTK output (frames per second){will use closest integer multiple of DT} [used only if WrVTK=2 or WrVTK=3]

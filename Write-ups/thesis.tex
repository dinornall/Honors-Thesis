...
To systematically generate the equations of motion for the multi-body wind turbine models, this project utilizes a custom symbolic derivation pipeline built upon the Python \texttt{sympy.physics.mechanics} module. The workflow begins by defining the system's reference frames, point locations, and generalized coordinates ($q(t)$). 

Once the kinematics are established, the kinetic energy ($\mathcal{T}$) and potential energy ($\mathcal{V}$) of the system are constructed symbolically. The pipeline then evaluates the Euler-Lagrange equations:
\begin{equation}
    \frac{d}{dt}\left(\frac{\partial\mathcal{L}}{\partial\dot{q}_{i}}\right)-\frac{\partial\mathcal{L}}{\partial q_{i}}=Q_{i,nc}
\end{equation}
Because deriving these equations manually becomes intractable for systems with 3D rotational kinematics and flexible bodies, the symbolic pipeline automatically executes the partial differentiation and algebraic simplification. Finally, the pipeline extracts the acceleration terms ($\ddot{q}$) to rearrange the highly non-linear ordinary differential equations into the standard matrix form:
\begin{equation}
    M(q,t)\ddot{q} = F(q,\dot{q},t)
\end{equation}
This automated generation creates a ``glass-box'' model where the explicit symbolic relationships between mass, geometry, and rotation are perfectly preserved for analysis.
...
While the nonlinear $M$ and $F$ matrices accurately describe the system's large-deflection dynamics, conducting a frequency and stability analysis requires linearizing these equations of motion about a steady operating point. For the models in this thesis, the equilibrium state ($q_0$) is defined by a constant nacelle tilt ($\theta_0$), a constant rotor speed ($\dot{\psi} = \Omega$), and zero initial blade deflection ($q_0 = 0$). 

The system's dynamics are evaluated by applying a small perturbation ($\delta q$) around this equilibrium:
\begin{equation}
    q(t) = q_0 + \delta q(t)
\end{equation}
Substituting this perturbed state into the nonlinear equations and performing a first-order Taylor series expansion yields the linearized equations of motion:
\begin{equation}
    M\delta\ddot{q} + C\delta\dot{q} + K\delta q = 0
\end{equation}
In this linear form, the mass matrix ($M$) represents the inertial properties of the system. The stiffness matrix ($K$) captures the structural elasticity as well as centrifugal and gravitational stiffening effects. Crucially, the damping matrix ($C$) contains both the structural and aerodynamic damping terms, alongside the skew-symmetric gyroscopic matrix generated by the rotating rotor. The interaction between the terms in these matrices—particularly the negative aerodynamic damping that occurs during stall—governs the aeroelastic instability of the turbine.

\subsection{Eigenvalue Analysis}
To extract the physical frequencies and damping characteristics from the linearized system, the second-order matrix equation is converted into a first-order state-space formulation:
\begin{equation}
    \dot{x} = Ax
\end{equation}
where the state vector is defined as $x = [\delta q, \delta\dot{q}]^T$, and the state matrix $A$ is constructed as:
\begin{equation}
    A = \begin{bmatrix} 0 & I \\ -M^{-1}K & -M^{-1}C \end{bmatrix}
\end{equation}
By computing the eigenvalues ($\lambda$) of the $A$ matrix, the dynamic response of the system can be isolated. The complex eigenvalues take the form:
\begin{equation}
    \lambda = \sigma \pm i\omega_d
\end{equation}
The imaginary component ($\omega_d$) represents the damped natural frequency of the mode, while the real component ($\sigma$) dictates the decay rate. The damping ratio ($\zeta$) is then calculated as:
\begin{equation}
    \zeta = \frac{-\sigma}{\sqrt{\sigma^2 + \omega_d^2}}
\end{equation}
To uncover the fundamental trends governing the wind turbine's behavior, this eigenvalue analysis is placed within a loop, systematically sweeping through a range of rotor speeds ($\Omega$) and wind velocities ($V$). The resulting frequencies and damping ratios are plotted to visualize phenomena such as frequency splitting and negative damping. Finally, to ensure the validity of the analytical model, these outputs are benchmarked against the eigenvalues generated by high-fidelity OpenFAST (ElastoDyn/AeroDyn) simulations under identical operating conditions.
...
The system defines the nacelle center of mass at offsets $(X_n, Z_n)$ and the hub at $(X_h, Z_h)$ relative to the pivot, with the rotor center of mass at radius $Z_r$. Inertial properties include the nacelle assembly ($J_{yyN}$) and the rotor principle inertias ($J_{xxR}, J_{yyR}, J_{zzR}$). The resulting mass matrix $M$ and forcing vector $F$ uniquely capture the centrifugal stiffening and gravitational restoring moments in their full symbolic forms:


The mass matrix $M$ and forcing vector $F$ for the tilting nacelle model are expressed below in their full symbolic forms.

\begin{equation}
    \label{eq:tilting_mass_matrix_downwind}
    \resizebox{0.95\textwidth}{!}{%
        $M = \begin{bmatrix}
                J_{yyb} \cos^{2}\psi + J_{yyn} + J_{zzb} \sin^{2}\psi + m_{b} (X_{h}^{2} + Z_{b}^{2} \cos^{2}\psi + 2 Z_{b} Z_{h} \cos\psi + Z_{h}^{2}) + m_{n} (X_{n}^{2} + Z_{n}^{2}) & - X_{h} Z_{b} m_{b} \sin\psi \\
...
\begin{equation}
    \label{eq:tilting_forcing_terms_downwind}
    \resizebox{0.95\textwidth}{!}{%
        $F = \begin{bmatrix}
                (J_{yyb} - J_{zzb} + Z_{b}^{2} m_{b}) \sin(2\psi) \dot{\psi} \dot{\theta} + X_{h} Z_{b} m_{b} \cos\psi \dot{\psi}^{2} + (X_{h} m_{b} + X_{n} m_{n}) g \cos\theta + 2 Z_{b} Z_{h} m_{b} \sin\psi \dot{\psi} \dot{\theta} + \frac{Z_{b} g m_{b} \sin(\psi - \theta)}{2} - \frac{Z_{b} g m_{b} \sin(\psi + \theta)}{2} - (Z_{h} m_{b} + Z_{n} m_{n}) g \sin\theta - c_{t} \dot{\theta} - k_{t} \theta \\
                \left(- J_{yyb} \cos\psi \dot{\theta}^{2} + J_{zzb} \cos\psi \dot{\theta}^{2} - Z_{b}^{2} m_{b} \cos\psi \dot{\theta}^{2} - Z_{b} Z_{h} m_{b} \dot{\theta}^{2} - Z_{b} g m_{b} \cos\theta \right) \sin\psi
            \end{bmatrix}$%
    }
\end{equation}
